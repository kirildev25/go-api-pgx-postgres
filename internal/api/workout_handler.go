...
import (
	...
	"github.com/kirildev25/go-api-pgx-postgres/internal/middleware"
...
func (wh *WorkoutHandler) HandleGetWorkoutByID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		utils.WriteJSON(w, http.StatusNotFound, utils.Envelope{"error": "failed to fetch the workout"})
		return
	}
...
func (wh *WorkoutHandler) HandleCreateWorkout(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		wh.logger.Printf("ERROR: json.NewDecoder: %v", err)
...
func (wh *WorkoutHandler) HandleUpdateWorkoutByID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			utils.WriteJSON(w, http.StatusNotFound, utils.Envelope{"error": "does not exist"})
			return
		}
	...
	if workoutOwner != currentUser.ID {
		utils.WriteJSON(w, http.StatusForbidden, utils.Envelope{"error": "Not Allowed"})
		return
	}

	existingWorkout.UserID = currentUser.ID
...
func (wh *WorkoutHandler) HandleDeleteWorkoutByID(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusNoContent, utils.Envelope{"deleted": workoutID})
